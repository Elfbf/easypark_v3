...
@section('page_title', 'Tipe Kendaraan')
...
@section('content')
    ...
    <nav style="display:flex;align-items:center;gap:6px;font-size:13px;margin-bottom:20px;">
        ...
        <span style="color:#D4D9E8;">/</span>
        <span style="color:#181D35;font-weight:600;">Tipe Kendaraan</span>
    </nav>

    {{-- ── Page Header ── --}}
    <div class="page-head">
        <div>
            <div class="page-title">Tipe Kendaraan</div>
            <div class="page-sub">Kelola data tipe kendaraan yang terdaftar dalam sistem</div>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <div>
                <div class="card-title">Daftar Tipe Kendaraan</div>
                <div class="card-sub">{{ $vehicleTypes->total() }} tipe kendaraan terdaftar dalam sistem</div>
            </div>

            <div style="display:flex;align-items:center;gap:10px;">
                {{-- Search --}}
                <form method="GET" action="{{ route('admin.vehicle-types.index') }}" style="display:flex;">
                    <div class="tb-search" style="width:220px;">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <circle cx="11" cy="11" r="8" />
                            <path d="M21 21l-4.35-4.35" />
                        </svg>
                        <input type="text" name="search" placeholder="Cari tipe kendaraan..."
                            value="{{ $search }}"
        ...
        @if ($vehicleTypes->isEmpty())
            ...
            <div style="padding:64px 24px;text-align:center;">
                ...
                <div
                    ...
                    @if ($search)
                        Tidak ada tipe kendaraan yang cocok
                    @else
                ...
                <div style="font-size:13px;color:#8A93AE;margin-bottom:22px;line-height:1.6;">
                    ...
                    @else
                        Tambahkan tipe kendaraan pertama untuk mulai mengelola data parkir
                    @endif
                </div>
                @if (!$search)
                    <button class="btn-primary" onclick="openModal()">
        ...
        @else
            <table class="data-table" id="vehicleTypeTable">
                <thead>
                    <tr>
                        ...
                        {{-- Nama Tipe --}}
                        <th style="padding:14px 16px;">Nama Tipe Kendaraan</th>
                ...
                <tbody>
                    @forelse($vehicleTypes as $index => $type)
                        @php
                            $vehicleCount = $type->vehicles()->count();
                        ...
                        <tr id="row-{{ $type->id }}">
                            ...
                            <td style="padding:14px 16px;text-align:center;">
                                <div style="display:flex;align-items:center;justify-content:center;gap:6px;">
                                    ...
                                    {{-- Edit --}}
                                    <button class="tb-btn" title="Edit tipe kendaraan"
                                    ...
                                    @if ($isProtected)
                                        <button class="tb-btn" title="Masih digunakan {{ $vehicleCount }} kendaraan"
                                            onclick="showToast('warning', 'Tipe &quot;{{ addslashes($type->name) }}&quot; masih digunakan oleh {{ $vehicleCount }} kendaraan. Hapus atau pindahkan data terlebih dahulu.')"
                                            style="width:32px;height:32px;border-radius:8px;
                                    ...
                                    @else
                                        <button class="tb-btn" title="Hapus tipe kendaraan"
            ...
            <div
                ...
                <span style="font-size:12.5px;color:#8A93AE;">
                    Menampilkan {{ $vehicleTypes->firstItem() }}–{{ $vehicleTypes->lastItem() }}
                    dari {{ $vehicleTypes->total() }} tipe kendaraan
                </span>
                ...
                @if ($vehicleTypes->hasPages())
                    <div style="display:flex;align-items:center;gap:6px;">
                        ...
                        @else
                            <a href="{{ $vehicleTypes->previousPageUrl() }}"
                        ...
                        {{-- Page numbers --}}
                        @foreach ($vehicleTypes->getUrlRange(1, $vehicleTypes->lastPage()) as $page => $url)
                            @if ($page == $vehicleTypes->currentPage())
                        ...
                        {{-- Next --}}
                        @if ($vehicleTypes->hasMorePages())
                            <a href="{{ $vehicleTypes->nextPageUrl() }}"
                               style="width:32px;height:32px;border-radius:8px;border:1.5px solid #D4D9E8;
                                      display:flex;align-items:center;justify-content:center;
                                      text-decoration:none;transition:border-color .2s,background .2s;"
                               onmouseover="this.style.borderColor='#3B6FD4';this.style.background='#F8FAFF'"
            ...
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
                ...
                <div>
                    ...
                    <div style="font-size:12px;color:#8A93AE;margin-top:2px;">
                        Buat tipe kendaraan baru dalam sistem
                    </div>
                </div>
            </div>

            <form action="{{ route('admin.vehicle-types.store') }}" method="POST" onsubmit="return validateAddForm()">
                @csrf

                <div style="margin-bottom:20px;">
                    <label style="display:block;font-size:13px;font-weight:600;color:#181D35;margin-bottom:8px;">
                        Nama Tipe <span style="color:#D92D20;">*</span>
                    </label>
                    <input type="text" name="name" id="addTypeName"
                        placeholder="Contoh: motor, mobil, truck..." autocomplete="off" required
        ...
        <div
            ...
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
                ...
                <div>
                    ...
                    <div style="font-size:12px;color:#8A93AE;margin-top:2px;">
                        Ubah nama tipe kendaraan yang dipilih
                    </div>
                </div>
            </div>
            ...
            <form id="editForm" method="POST" onsubmit="return validateEditForm()">
                ...
                <div style="margin-bottom:20px;">
                    ...
                    <input type="text" name="name" id="editTypeName" required autocomplete="off"
                        style="width:100%;height:42px;border:1.5px solid #D4D9E8;
                               border-radius:10px;padding:0 14px;outline:none;
                               font-family:'DM Sans',sans-serif;font-size:13.5px;
                               color:#181D35;background:#fff;
...
@push('scripts')
    <script>
        // ═══════════════════════════════
        // DATA TIPE UNTUK VALIDASI DUPLIKAT
        // ═══════════════════════════════
        const existingTypes = @json($vehicleTypes->pluck('name', 'id'));
        ...
        function validateAddForm() {
            ...
            if (!val) return true;
            const duplicate = Object.values(existingTypes).some(n => n.toLowerCase() === val);
            if (duplicate) {
                showAddError('Tipe kendaraan dengan nama ini sudah ada.');
                return false;
            }
            return true;
        }
        ...
        function openEditModal(id, name) {
            ...
            document.getElementById('editForm').action = '/admin/vehicle-types/' + id;
            document.getElementById('modalEdit').style.display = 'flex';
            clearEditError();
            setTimeout(() => document.getElementById('editTypeName').focus(), 100);
        }
        ...
        function clearEditError() {
            ...
            document.getElementById('editTypeName').style.borderColor = '#D4D9E8';
            document.getElementById('editTypeName').style.boxShadow = 'none';
        }

        function validateEditForm() {
            const val = document.getElementById('editTypeName').value.trim().toLowerCase();
            if (!val) return true;
            const duplicate = Object.entries(existingTypes)
                .some(([id, name]) => parseInt(id) !== currentEditId && name.toLowerCase() === val);
            if (duplicate) {
                showEditError('Tipe kendaraan dengan nama ini sudah ada.');
                return false;
            }
            return true;
        }
